...
stock Action AddAttribute(int slot, int client, char[] attrib, char[] plugin, char[] value, int weapon = -1, bool whileActive = false)
{
	if (!IsValidClient(client))
	{
	...
	else if (!StrEqual(plugin, "tf2items", false))
	{
		...
		if (StrEqual(plugin, "tf2attributes", false))
		{
			TF2Attrib_SetByName(weapon, attrib, StringToFloat(value));
		}
		else
		{
			TF2Attrib_SetByName(weapon, attrib, Float:StringToInt(value));
		}
	}
